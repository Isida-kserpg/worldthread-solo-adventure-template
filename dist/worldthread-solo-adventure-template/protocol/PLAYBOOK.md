...
1. **程式擲骰（優先）**：依你所在環境的執行能力擇一呼叫，並在回覆中逐字引用其輸出的 JSON 行——能執行 Node 18+ 用 `node tools/dice.mjs 2d6+3`；能執行 Python 3.8+ 用 `python tools/dice.py 2d6+3`（部分系統指令名為 `python3`）。兩支工具輸出契約完全相同，任選其一即可。
2. **玩家擲骰**：無執行能力則請玩家擲骰（實體骰或自行執行任一工具）並回報。
3. **AI 自骰（最終降級）**：以上皆不可行且玩家同意時，才可自行產生骰值；必須在敘事前明白告知玩家這是 AI 自骰、公正產生不得偏袒任何結果，事件 `source` 如實記為 `"ai"`，不得偽稱其他來源。玩家可隨時否決並改用其他方式。

骰值依 `DATA-SCHEMA.md` 以 `kind: "roll"` 事件記入日誌，`source` 如實填寫。玩家永遠可以選擇自行擲骰。
