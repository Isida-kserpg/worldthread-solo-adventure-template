...
> 每回合末主持人會給一行存檔確認（例：`✦ 進度已存`）——看到它就可以安全關閉對話。
...
- **系統雜訊**：安靜
  <!-- 安靜（預設）＝敘事零系統雜訊，只在必要時附極簡標記（骰值、無法寫檔的 STATE-UPDATE）；
       標準＝骰值加簡短 OOC 說明；除錯＝顯示 revision／檔名／工具呼叫（供排錯）。
       回合末存檔確認（✦ 進度已存）與判定時的資源選項提示不受此層級影響，所有層級都會顯示。
