...
**備份與分享戰役**：整個資料夾就是完整存檔。複製或壓縮整個資料夾即為**完整備份**——含 `game/private/` 導演祕密，僅供你自己保存；匯出前順手記下 `template.json` 的版本，日後在新版範本上續玩時可對照。舊存檔在新版範本上可直接續玩：存檔格式自 0.5.0 起穩定、無需任何轉檔（相容性原則見 `protocol/DATA-SCHEMA.md`〈版本相容性與寬容讀取〉）。把**進行中的戰役分享給他人閱讀**（或讓對方另起爐灶）時，先複製一份並**整個刪除 `game/private/`** 再交出——導演祕密（前線、伏筆、主線大綱）外流會毀掉這場戰役的續玩體驗；若是**同一戰役換人接手主持**，`game/private/` 正是接手所需的導演連續性資料，應隨檔以信任管道轉移、不要刪除。剛解壓、尚未開局的發行包不在此列：內附「霧渡口」範例的導演資料本來就是範例的一部分，直接整包分享即可。備份複製回來（或解壓）後照常開局，續玩判斷自動生效。
