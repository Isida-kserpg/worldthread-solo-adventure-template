...
版本：`0.6.2`。這是一個可複製、解壓後即可交給具資料夾讀寫能力之 AI 服務使用的繁體中文單人 TRPG 範本（另含可選啟用的實驗性共窗多人）；不綁定特定模型或平台。
...
**備份與分享戰役**：整個資料夾就是完整存檔。複製或壓縮整個資料夾即為**完整備份**——含 `game/private/` 導演祕密，僅供你自己保存；匯出前順手記下 `template.json` 的版本，日後在新版範本上續玩時可對照。把**進行中的戰役分享給他人閱讀**（或讓對方另起爐灶）時，先複製一份並**整個刪除 `game/private/`** 再交出——導演祕密（前線、伏筆、主線大綱）外流會毀掉這場戰役的續玩體驗；若是**同一戰役換人接手主持**，`game/private/` 正是接手所需的導演連續性資料，應隨檔以信任管道轉移、不要刪除。剛解壓、尚未開局的發行包不在此列：內附「霧渡口」範例的導演資料本來就是範例的一部分，直接整包分享即可。備份複製回來（或解壓）後照常開局，續玩判斷自動生效。
