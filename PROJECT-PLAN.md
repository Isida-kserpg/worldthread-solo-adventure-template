...
> 狀態：`0.6.0` 已發行；`0.6.x` 收斂（驗證與固本，先不進 `0.7.0`）進行中，定案見〈階段 7〉。歷版摘要見下表——結構化狀態列為 2026-07-19 定案（C2）於本檔試點，取代逐版附加長句，防版本字串遺漏未更新。
...
- [ ] **收版 `0.6.1`**（D4）：條文包＋文件債落地後收版（`template.json`／發行包 README 版本行／DESIGN 狀態列／本表同步）；補測發現缺陷視情況 `0.6.2+`。
- [ ] **`0.6.0` 補測輪（A1 拆兩輪）**：Run 3＝`archive/` 觸發劇本（playground 本地素材，A3；dist 第二內附場景另議）＋短團收尾全流程（完結→尾聲→續玩接新弧）；Run 4＝共窗多人 Hot Seat 雙玩家＋host-log 刻意例外情境；檢核清單比照 0.5.0 模式置 playground 本機不追蹤檔。A2 定調：回應速度／出戲感僅由使用者真人跑團主觀回報、不再嘗試自動化評測。結果由使用者驗收後回收；若 Run 3 仍未觸發 `archive/`，視為判準文件仍模糊的訊號、優先文件澄清而非加排播測。
