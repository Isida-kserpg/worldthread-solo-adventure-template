...
> 狀態：`0.6.2` 已發行；`0.6.x` 收斂（驗證與固本，先不進 `0.7.0`）進行中，定案見〈階段 7〉。歷版摘要見下表——結構化狀態列為 2026-07-19 定案（C2）於本檔試點，取代逐版附加長句，防版本字串遺漏未更新。

| 版本 | 發行日 | 本版重點 |
| --- | --- | --- |
| `0.1.0` | 2026-07-14 | 首發：可攜範本骨架、`protocol/`、平台中立擲骰工具 |
| `0.2.0` | 2026-07-14 | 擲骰 Python 對等工具、雙引擎契約測試、AI 自骰最終降級 |
| `0.3.0` | 2026-07-15 | Fate Core／FAE（CC-BY 3.0）規則範例庫與開局宣告 |
| `0.4.0` | 2026-07-16 | 敘事沉浸分層（IC／OOC、系統雜訊三級）＋敘事風格萃取範本 |
| `0.5.0` | 2026-07-16 | 外部遊玩反饋改善（見〈階段 5〉） |
| `0.6.0` | 2026-07-18 | 決策看板 14 題定案範圍（見〈階段 6〉）；套件更名 `worldthread-core` |
| `0.6.1` | 2026-07-19 | 0.6.x 防呆條文包＋文件債清償（見〈階段 7〉） |
| `0.6.2` | 2026-07-19 | correction 撤銷語意攤平＋寫入即驗證可解析＋狀態健檢工具 healthcheck（源自 journal 外部玩家回饋 1／2） |
...
- [x] **0.5.0 實測清單**：產出六機制（rules-quickref 強制核對、單人調整、存檔確認、inventory/quests、system 容器、實體分層＋回應前實體核對）逐項檢核清單（勾選＋一句話、≤15 條）；置本機不追蹤檔自用，回饋由使用者手動摘要帶回（不含逐字戰役內容）。**回收完成（2026-07-18，Run 2 沈汲／霧渡口／共同創角）**：六機制全項 ✅，逐一核對 `events.jsonl`／state 檔一致、無新缺陷、不需 `fix/` 分支；唯一 ⚠️ 為既有反饋「短期團缺收尾機制」得第二資料點印證（見下〈短團收尾〉工作項）。
...
- [x] **短團收尾機制（2026-07-18 實測回饋衍生、mini-ceremony 四點定案）**：使用者定案「完整版納入 0.6.0」。定案：①完結標記＝`world.json` 頂層選用 `campaign_status`（`active` 預設／`concluded`）＋選用 `concluded_at_event_id`，缺欄＝active（舊格式相容）、供可視化讀取、**完結≠封存且可續玩**（回 active／新弧，事件與 state 延續不重置）；②summaries 完結時條件式新增〈尾聲〉節（第 4 節，未完結無此節）、「未決線頭」改記最終處置；③新增導演私有 `game/private/director/campaign-arc.md`（主線大綱＝大方向／潛在尾聲形狀／主線里程碑，確保發散支線仍能抵達尾聲），PLAYBOOK〈初始化〉補設計、新增〈戰役收尾〉節（觸發＝主線抵高潮／玩家宣告／長度達成主持人提議、絕不單方結束→收束前線與線頭→尾聲敘事→終局存檔→續玩接新弧）；④session-brief B 段新增「戰役長度預期」（單次／短期／長期，預設長期）供配速。落地檔：DATA-SCHEMA（世界／摘要／封存／新增戰役主線大綱節）、PLAYBOOK（初始化第 6 步／每回合第 6 步配速／新增戰役收尾節）、session-brief、新增 campaign-arc.md 範例、verify 腳本結構斷言、DESIGN 結構樹。紅線守：不擴 visibility enum、沿用舊格式相容、公私分層、dist-only。
- [x] **多人擴充**：目標形態＝「bot 中繼 Discord／Telegram 頻道 ↔ AI」遠端多人（bot 為另開專案，引用本範本為核心模組使溝通架構一致）；分人方式未定（第三人稱自標／論壇團式主持統整為候選）。0.6.0 先做架構研究（分人方式、訊息流、本範本需預留的接縫），研究結論可行即動工最小多人協定並依結果起草 §1 定位文字交使用者定案；**動 schema 前受局部閘門約束（須先核對 0.5.0 實測回饋，並顧可視化專案讀取契約相容）**。若定位定案為支援多人，**套件改名（移除 `-solo-`）綁定同一版本一起做**（dist 資料夾名＝manifest name＝ZIP 根目錄名三處一次協調變更；GitHub repo 改名自動重導、既有 tag 不動）（2026-07-17 使用者提出、綁定原則定案）。**架構研究完成＋定案（2026-07-17）**：分人方式＝第三人稱自標（bot 側可 user→pc-id 自動歸屬強化）＋戰鬥／高風險判定分層疊加點名 spotlight；訊息流＝收集窗口聚合結算（即時逐條因結構性踩 revision 衝突面排除；窗口參數屬 bot 專案）；擲骰＝bot 側執行 `dice.mjs` 頻道公開原始輸出（信任邊界升級、嚴守逐字回填 `source: "tool"`）；**PC 間祕密＝需要**——未來走與 visibility 正交的 `visible_to: [pc-ids]` 選用欄、**絕不擴充 visibility 三值 enum**（可視化契約相依），受局部閘門、本版只標記不落地；§1 定位＝D2（單人為主＋雙定位預告）。0.6.0 最小動作集已落地：DATA-SCHEMA 多人註記擴充（bot 中繼目標形態）＋events 選用 `actor` 鍵（行動者歸屬，與 `updates` 正交、additive 不受閘門）＋revision 仲裁歸屬原則一句。**共窗多人（實驗，Hot Seat）落地（2026-07-18 決策看板 ceremony，7 題全採建議定案）**：使用者定案在本範本加入可選啟用的實驗性本地多人。A1 並存雙路徑（Hot Seat＝實驗本地形態落地本範本／bot 中繼＝遠端目標形態仍另開專案），§1 定位改為誠實反映兩條路徑（理由：新增實驗本地形態不推翻遠端目標）；A2 session-brief B 段新增〈遊玩模式〉欄（單人預設／共窗多人，留空即單人零污染）；A3 共窗多人啟用時每 PC 存 `game/state/characters/<pc-id>.json`（落實既有預留、gated 到實驗模式，單人維持 `character.json` 不遷移）；A4 沿用既有 events `actor` 鍵（本模式建議必填）＋第三人稱自標＋PLAYBOOK 補 spotlight，零新 schema；A5 誠實面對單窗物理限制——**不支援** PC 間對在場玩家隱藏資訊，導演祕密仍走 `private/director`，PC 間祕密機制化留待遠端形態，**visibility 三值 enum 不動**（守死紅線）；A6 不新增 per-PC 命名空間，IC 點名/第三人稱指涉、OOC 全隊可見；B1 中文稱「共窗多人（實驗，Hot Seat）」、簡稱 Hot Seat 模式。落地檔：PLAYBOOK（新增〈共窗多人〉節＋每回合 spotlight 一句）、DATA-SCHEMA（多人架構段改寫＋events actor 註記）、session-brief（遊玩模式欄）、PROJECT-DESIGN §1 定位＋§2 結構樹、README B 段列舉。`characters/` 屬遊玩期產生、不入 dist（verify 無需變更）。
- [x] **套件改名（worldthread-core）**：**改名決策更新（2026-07-17 使用者定案，取代「延至雙定位版」原議）**：改名**提前至 0.6.0 執行**——中性化名稱不承諾多人、無名實不符空窗，且外部引用累積最少的時點成本最低；新名 **`worldthread-core`**（生態系核心定位：衍生專案 bot／可視化／web UI 引用本專案為核心模組），dist 資料夾＝manifest name＝ZIP 根目錄＝GitHub repo 四處一致變更；§1（含生態系定位句）／§6 文字同批落地。舊 tag 與已發行 ZIP 不動。

## 階段 7：`0.6.x` 收斂（2026-07-19 決策看板 15 題定案；先不進 `0.7.0`）

前置研究＝0.7.0 方向研究報告（本機 `WORLDTHREAD-ROADMAP-RESEARCH.local.md`，2026-07-18，內外部九研究 lane＋對抗查證）。核心結論：`0.6.0` 四項新機制（短團收尾、共窗多人 Hot Seat、host-log、campaign-arc）零實測資料點、其中 `campaign_status` 已是可視化衍生專案的讀取契約——本階段專注把 `0.6.0` 做實；功能面擴充維持訊號驅動延後（〈後續擴展〉backlog 不動）。決策看板 15 題（A1–A3 補測／B1–B6 條文包／C1–C2 文件債／D1–D4 其他）全採建議定案。

- [ ] **防呆條文包（B1–B5＋D1）**：DATA-SCHEMA〈通用規則〉補原子寫入建議慣例（B1，消除下游讀取者 torn read）；PLAYBOOK〈初始化〉補「玩家隨時可 OOC 喊停／迴避、主持人立即淡出不追問理由」明文句（B2）；DATA-SCHEMA〈場景與 `scene_id`〉＋PLAYBOOK 每回合第 6 步補**場景切換判準**（B3——兩輪播測 `archive/` 未觸發的根因＝判準模糊）；〈摘要〉補玩家人工編輯優先句（B4 核對結果：壓縮時機已錨定場景邊界、僅此句缺）；RAG-PROTOCOL 失效判準補「嵌入模型／版本無法確認即失效」（B5；`embedding_model` 選配欄延後、屬 schema 變更另議）；發行包 README 補戰役備份／分享慣例（D1：完整備份／閱讀分享刪 `game/private/`／換手主持信任轉移三情境＋匯出前記版本提醒）。（本項已隨 PR #25 落地並經三 lens 對抗審查，待使用者驗收勾選。）
- [ ] **文件債（C1／C2／D3＋版本行修正）**：DESIGN 狀態列版本字串修正（原誤留「`0.5.0` 已發行」——`0.6.0` 收版時漏改版本本體的事實錯誤）；DESIGN §6 補戰役收尾指路句＋§8 補收尾驗收句（C1 最小組合，指路寫法防雙準源）；PLAN 狀態列結構化表格試點（C2，本檔）；Claude 專屬 rules 檔補 fable 不可用時架構設計 fallback 句（D3）。（本項已隨 PR #24 落地，待使用者驗收勾選。）
- [ ] **收版 `0.6.1`**（D4）：條文包＋文件債落地後收版（`template.json`／發行包 README 版本行／DESIGN 狀態列／本表同步）；補測發現缺陷視情況 `0.6.2+`。（2026-07-19 收版落地，待使用者驗收勾選。）
- [ ] **`0.6.0` 補測輪（A1 拆兩輪）**：Run 3＝`archive/` 觸發劇本（playground 本地素材，A3；dist 第二內附場景另議）＋短團收尾全流程（完結→尾聲→續玩接新弧）；Run 4＝共窗多人 Hot Seat 雙玩家＋host-log 刻意例外情境；檢核清單比照 0.5.0 模式置 playground 本機不追蹤檔。A2 定調：回應速度／出戲感僅由使用者真人跑團主觀回報、不再嘗試自動化評測。結果由使用者驗收後回收；若 Run 3 仍未觸發 `archive/`，視為判準文件仍模糊的訊號、優先文件澄清而非加排播測。（**Run 3/4 已於 2026-07-19 執行**：playground 同步 0.6.1 協定後跑；archive 於 Run 3 場景切換確實觸發、短團收尾＋續玩新弧全流程走完，Run 4 共窗多人＋host-log 例外皆觸發；結果檔 `PLAYTEST-RESULT-run3/run4.local.md`，經三 lens 對抗審查修正。判讀紀律「archive 觸發＝不加排第四輪」已滿足；待使用者驗收回收。）
- [ ] **journal 相依守則（D2＋B6）**：局部閘門延伸——僅動 `system`／`entities`／visibility／`campaign_status` 的工作項須核對 journal（可視化衍生專案）回饋，並設等待期限（動工前無回饋＝按現況推進、事後補核）；**journal 屬另開專案，core 端不逕改其內容**；core 發行檢查清單（本機檔）加 sibling 消費者同步提醒行（B6）。
- [ ] **`0.6.2` journal 回饋落地（2026-07-19 定案）**：源自 worldthread-journal `CORE-FEEDBACK.local.md`（cabalist 外部玩家 core v0.6.0 實測）三項——**回饋 1**＝correction 撤銷語意攤平（`corrects` 一律指向非-correction 原事件、同一原事件最新 correction 為有效版本；DATA-SCHEMA 事件日誌節改寫＋撤銷正例）；**回饋 3**（次要，隨回饋 1）＝溯源欄位指向仍有效事件；**回饋 2**＝PLAYBOOK〈每回合〉第 5 步補「寫入即驗證可解析」＋新增雙 mjs/py 狀態健檢工具 `tools/healthcheck.*`（＋fixtures＋`scripts/test-healthcheck-contract.ps1`＋CI／verify-package 掛載）。收版四處同步至 `0.6.2`。（本項已隨 `fix/0.6.2-journal-feedback` 落地、verify-package＋雙契約測試綠、並經三 lens 對抗審查修正 correction 範例衝突與測試覆蓋缺口，待使用者驗收勾選。）
